...
for (s in 1:nsim) {
  ...
  for (i in 1:d) {
    ...
    tmp <-which(abs(R[1:KT]) < 2*sqrtLog  )
  ...
  # blocksize
  #生成Bootstrap样本
  y.star <- matrix(rep(NA,d),nrow=1,ncol=d)
  
  while (nrow(y.star) < (N+trainingSize/2)) {
    idx.start <- sample.int(trainingSize, 1, replace=TRUE) 
    #生成{1,...,T}的离散均匀分布值，作为随机块的起点
    idx.len <- rgeom(1, 1/blocksize) + 1        
    idx <- idx.start:(idx.start + idx.len)
    idx <- (idx - 1) %% trainingSize + 1
